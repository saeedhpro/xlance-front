...
<div class="p-4 relative mt-10 bg-white rounded-lg shadow-lg">
  <div class="pb-2 mb-5 flex border-b-2 border-gray-300">
    <div class="pb-3 text-sm text-gray-500 ir-medium text-lg"  @click="status='1'" :class="{'select-purple' : status==='1'}">شبکه اجتماعی</div>
  </div>
...
<!--      <PostFormat v-for="(p , i ) in getMyPost" :key="i" :post="p" class="w- h-64"/>-->
      <nuxt-link :to="`/socialMedia/${p.id}`" v-for="(p , i ) in getMyPost" :key="i">
        <img class="m-1 rounded-lg shadow-lg" style="width: 90px; height: 90px;" :key="i" :post="p" :src="p.media.path"/>
      </nuxt-link>
    </div>
  </div>
</div>
