...
<!--        </div>-->
    ...
    <div class="flex-col">
      <div class="flex px-1 py-1 border-dashed border-2 border-gray-400 rounded-lg text-center float-left" style="align-items: center;">
        ...
        <span class="ir-light text-gray-600 text-sm"><slot name="name-inf">پروژه</slot></span>
      </div>
      <div class="mt-12" style="    display: flex;
    flex-wrap: wrap;
    justify-content: flex-end;">
        <nuxt-link :to="url2" v-text="btnName2" class="sm:p-2 p-1 sm:mx-2 my-1 btn-size bg-gray-300 rounded-lg text-gray-700 sm:text-base text-sm  hover:bg-greenKam hover:text-greentype">مشاهده پروژه ها</nuxt-link>
        <nuxt-link :to="url" v-text="btnName" class="sm:p-2 p-1 btn-size my-1 bg-gray-300 rounded-lg text-gray-700 sm:text-base text-sm hover:bg-greenKam hover:text-greentype">مشاهده پروژه ها</nuxt-link>
      </div>
    </div>
  </div>
